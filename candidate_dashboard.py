...
# ==============================================================================
# 1. DEPENDENCIES & HELPER FUNCTIONS
# ==============================================================================
...
def parse_and_store_resume(file_input, file_name_key='default', source_type='file'):
    ...
    default_structured_experience = [
        {"company": "Prgayan AI", "role": "AIML Engineer", "from_year": "2025", "to_year": "Present", "ctc": "Negotiable", "responsibilities": "Developing and deploying AI/ML models for NLP and Computer Vision projects."},
        {"company": "DataStart Innovations", "role": "Junior Developer", "from_year": "2022", "to_year": "2024", "ctc": "$60k", "responsibilities": "Developed ETL pipelines using Python and SQL."}
    ]
    
    default_structured_education = [
        {"degree": "M.Sc. Computer Science", "college": "University of Excellence", "university": "City University", "from_year": "2020", "to_year": "2022", "score": "8.5", "type": "CGPA"},
        {"degree": "B.Tech. Information Technology", "college": "College of Engineering", "university": "State University", "from_year": "2016", "to_year": "2020", "score": "75", "type": "Percentage"}
    ]
    
    default_structured_certifications = [
        {"title": "AWS Certified Cloud Practitioner", "given_by": "Amazon Web Services", "issue_date": "2023-10-01"}
    ]
    
    parsed_data = {
        "name": name_from_file, "email": "candidate@example.com", "phone": "555-123-4567",
        ...
        "experience": default_structured_experience, "structured_experience": default_structured_experience, 
        "education": default_structured_education, "structured_education": default_structured_education, 
        "certifications": default_structured_certifications, "structured_certifications": default_structured_certifications, 
...
def cv_management_tab_content():
    ...
    default_parsed = {
        ...
        "structured_experience": [], "structured_certifications": [], "structured_education": []
    }
    ...
    with st.form("cv_builder_form", clear_on_submit=False):
        ...
        with col1:
            st.session_state.cv_form_data['name'] = st.text_input("Full Name", value=st.session_state.cv_form_data['name'], key="cv_name_input").strip() 
        with col2:
            st.session_state.cv_form_data['email'] = st.text_input("Email Address", value=st.session_state.cv_form_data['email'], key="cv_email_input").strip() 
        with col3:
            st.session_state.cv_form_data['phone'] = st.text_input("Phone Number", value=st.session_state.cv_form_data['phone'], key="cv_phone_input").strip() 
        
        col4, col5 = st.columns(2)
        with col4:
            st.session_state.cv_form_data['linkedin'] = st.text_input("LinkedIn Profile URL", value=st.session_state.cv_form_data.get('linkedin', ''), key="cv_linkedin_input").strip() 
        with col5:
            st.session_state.cv_form_data['github'] = st.text_input("GitHub Profile URL", value=st.session_state.cv_form_data.get('github', ''), key="cv_github_input").strip() 
        
        st.session_state.cv_form_data['personal_details'] = st.text_area("Professional Summary (A brief pitch about yourself)", value=st.session_state.cv_form_data.get('personal_details', ''), height=100, key="cv_personal_details_input").strip() 
        ...
        new_skills_text = st.text_area("Technical and Soft Skills", value=skills_text, height=100, key="cv_skills_input_form")
        ...
        new_projects_text = st.text_area("Projects (Name, Description, Technologies)", value=projects_text, height=100, key="cv_projects_input_form")
        ...
        new_strength_text = st.text_area("Key Personal Qualities", value=strength_text, height=70, key="cv_strength_input_form")
    ...
    def add_education_entry():
        ...
        new_entry = {
            "degree": degree_val, "college": college_val, "university": university_val,
            "from_year": from_year_val, "to_year": to_year_val, "score": score_val, "type": score_type_val
        }
    ...
    with st.container(border=True):
        ...
        with col_d:
            st.text_input("Degree/Qualification", value=st.session_state.get("temp_edu_degree_key", ""), key="temp_edu_degree_key", placeholder="e.g., M.Sc. Computer Science")
            
        with col_c:
            st.text_input("College Name", value=st.session_state.get("temp_edu_college_key", ""), key="temp_edu_college_key", placeholder="e.g., MIT, Chennai")
            
        st.text_input("University Name", value=st.session_state.get("temp_edu_university_key", ""), key="temp_edu_university_key", placeholder="e.g., Anna University")
        ...
        with col_s:
            st.text_input("CGPA or Score Value", value=st.session_state.get("temp_edu_score_key", ""), key="temp_edu_score_key", placeholder="e.g., 8.5 or 90")
        with col_st:
            st.selectbox("Type", options=["CGPA", "Percentage", "Grade"], index=["CGPA", "Percentage", "Grade"].index(st.session_state.get("temp_edu_type_key", "CGPA")), key="temp_edu_type_key", label_visibility='collapsed')
    ...
    def add_certification_entry():
        ...
        new_entry = {"title": title_val, "given_by": given_by_val, "issue_date": issue_date_val}
    ...
    with st.container(border=True):
        ...
        with col_t:
            st.text_input("Certification Title", value=st.session_state.get("temp_cert_title_key", ""), key="temp_cert_title_key", placeholder="e.g., Google Cloud Architect")
            
        with col_g:
            st.text_input("Issuing Organization", value=st.session_state.get("temp_cert_given_by_key", ""), key="temp_cert_given_by_key", placeholder="e.g., Coursera, AWS, PMI")
            
        col_d, _ = st.columns(2)
        with col_d:
            st.text_input("Issue Date (YYYY-MM-DD or Year)", value=st.session_state.get("temp_cert_issue_date_key", str(date.today().year)), key="temp_cert_issue_date_key", placeholder="e.g., 2024-05-15 or 2023")
    ...
    def add_experience_entry():
        ...
        new_entry = {
            "company": company_val, "role": role_val, "from_year": from_year_val, 
            "to_year": to_year_val, "ctc": ctc_val, "responsibilities": responsibilities_val
        }
    ...
    with st.container(border=True):
        ...
        with col_c:
            company_val = st.text_input("Company Name", value=st.session_state.get("temp_exp_company_key", ""), key="temp_exp_company_key", placeholder="e.g., Google")
            
        with col_r:
            role_val = st.text_input("Role/Title", value=st.session_state.get("temp_exp_role_key", ""), key="temp_exp_role_key", placeholder="e.g., Data Scientist")
        ...
        with col_fy:
            ...
            try: from_year_index = from_year_options.index(current_from_year) 
            except ValueError: from_year_index = 0
            st.selectbox("From Year", options=from_year_options, index=from_year_index, key="temp_exp_from_year_key")
            
        with col_ty:
            to_year_options = ["Present"] + year_options
            try: to_year_index = to_year_options.index(current_to_year)
            except ValueError: to_year_index = 0
            st.selectbox("To Year", options=to_year_options, index=to_year_index, key="temp_exp_to_year_key")
            
        with col_c3:
            ctc_val = st.text_input("CTC (Annual)", value=st.session_state.get("temp_exp_ctc_key", ""), key="temp_exp_ctc_key", placeholder="e.g., $150k / 20L INR")

        responsibilities_val = st.text_area("Key Responsibilities/Achievements (Brief summary)", value=st.session_state.get("temp_exp_responsibilities_key", ""), height=70, key="temp_exp_responsibilities_key")
...
def filter_jd_tab_content():
    ...
    with st.form(key="jd_filter_form"):
        ...
        with col1:
            selected_skills = st.multiselect("Skills Keywords (Select multiple)", options=unique_skills_list, default=st.session_state.get('last_selected_skills', []), key="candidate_filter_skills_multiselect", help="Select one or more skills. JDs containing ANY of the selected skills will be shown.")
...
def candidate_dashboard():
    ...
    tab_cv_mgmt, tab_parsing, tab_jd_mgmt, tab_batch_match, tab_filter_jd, tab_chatbot, tab_interview_prep = st.tabs([
        "✍️ CV Management", "📄 Resume Parsing", "📚 JD Management", "🎯 Batch JD Match", "🔍 Filter JD", "💬 Resume/JD Chatbot (Q&A)", "❓ Interview Prep"           
    ])
    ...
    with tab_parsing:
        ...
        if input_method == "Upload File":
            ...
            uploaded_file = st.file_uploader("Choose PDF, DOCX, TXT, JSON, MD, CSV, XLSX file", type=["pdf", "docx", "txt", "json", "md", "csv", "xlsx", "markdown", "rtf"], accept_multiple_files=False, key='candidate_file_upload_main')
            ...
            pasted_text = st.text_area("Copy and paste your entire CV or resume text here.", value=st.session_state.get('pasted_cv_text', ''), height=300, key='pasted_cv_text_input')
            ...
            if st.session_state.get('interview_qa'):
                ...
                with st.form("interview_practice_form"):
                    ...
                    for i, qa_item in enumerate(st.session_state.interview_qa):
                        ...
                        answer = st.text_area("Your Answer for Q{i+1}", value=st.session_state.interview_qa[i]['answer'], height=100, key=f'answer_q_{i}', label_visibility='collapsed')
