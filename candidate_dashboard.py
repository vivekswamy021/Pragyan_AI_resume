...
def jd_management_tab_candidate():
    ...
    def extract_jd_from_linkedin_url(url):
        ...
        Company: MockCorp
        Location: Remote
        Job Summary: We are seeking a highly skilled **{role}** to join our team. The ideal candidate will have expertise in {skills_str}. Must be focused on **{focus}**. This is a Full-time position.
        """
...
# --- CORE UTILITY DECLARATIONS ---
# Ensure these fallbacks are defined to prevent internal tab cross-talk failures
def clear_interview_state(mode):
    """Safely purges interview state lists based on structural modes."""
    if mode == 'resume':
...
def display_evaluation_form(mode, qa_data_list, context_for_eval):
    ...
    if qa_data_list:
        ...
        st.markdown("---")
        if st.button(f"🗑️ Clear {mode.upper()} Practice Session", key=f"clear_interview_prep_session_{mode}"):
...
def interview_preparation_tab():
    """Interview Preparation Tab Logic with sub-tabs."""
    ...
    with tab_resume:
        ...
        if not question_section_options:
            st.error("No relevant sections found in the parsed resume for question generation.")
            return
        ...
        if st.button("Generate JD Questions", key='iq_btn_jd_c', use_container_width=True):
            ...
            with st.spinner(f"Generating questions based on JD: {selected_jd_name}..."):
                try:
                    clear_interview_state('jd')
                    raw_questions_response = generate_interview_questions(
                        source_data=selected_jd.get('content', ''), 
                        source_type='jd', 
                        identifier=selected_jd.get('name', 'N/A')
                    )
...
def cover_letter_tab():
    """Tab layout managing cover letter parsing, strategy customization, and exports."""
    ...
    if st.button("🚀 Process & Generate Cover Letter", type="primary", use_container_width=True, key="cl_tab_v2_master_process_trigger_btn"):
        if not resume_payload_text.strip():
            ...
            with st.spinner("Processing dimensions and drafting tailored validation correspondence..."):
                st.session_state.cl_v2_cached_output_string = ""
...
def gap_analysis_tab():
    ...
    st.markdown("This tool analyzes your biggest skill gaps from your best-matched Job Description and suggests a course plan.")
    st.markdown("---")
...
def resume_qa_content():
    ...
    if not is_data_loaded_and_valid:
        st.warning("⚠️ **Q&A Disabled:** Please parse a valid resume first.")
        return
...
def jd_qa_content():
    ...
    jd_names = [jd.get('name') for jd in st.session_state.candidate_jd_list if jd.get('name')]
    selected_jd_name = st.selectbox("Select Job Description", options=jd_names, key="selected_jd_for_qa")
    ...
    st.info(f"Chatting about: **{selected_jd_name}**")
    st.markdown("---")
    ...
    if prompt := st.chat_input(f"Ask about the requirements...", key="jd_qa_input"):
    ...
    if current_jd_history:
        st.markdown("---")
        if st.button(f"🗑️ Clear Chat History for selected JD", key="clear_jd_chatbot_history"):
...
def candidate_dashboard(go_to=None):
    """Set page structure parameters and distribute tabs execution layers."""
    # Set page configuration once at setup initialization boundary
    ...
    # FIXED: Corrected initialization key mismatch so CV builder form can write smoothly
    ...
    with tab_with_chatbot if 'tab_with_chatbot' in locals() else tab_chatbot:
        chatbot_tab_content()
...
if __name__ == '__main__':
    # FIXED: Signature now runs seamlessly without argument constraints
    candidate_dashboard()
